...
# ── Test 10: Amend across days uses committer date ────────────────

test_amend_cross_day() {
  echo "-- test_amend_cross_day"
  setup_repo
  ...
  # Amend on day 2 (author date preserved, committer date updates)
  echo "amended" > file.txt
  git add file.txt
  GIT_AUTHOR_DATE="2025-01-15 10:00:00 +0000" \
  GIT_COMMITTER_DATE="2025-01-16 09:00:00 +0000" \
  git commit -q --amend -m "Initial work (amended)"

  # Day 1: should be empty (committer date is day 2)
  local output_day1
  output_day1="$(run_git_done --since=2025-01-15T00:00:00 --until=2025-01-16T00:00:00)"
  assert_not_contains "amend: day 1 empty" "$output_day1" "Initial work"

  # Day 2: should show the amended commit
  local output_day2
  output_day2="$(run_git_done --since=2025-01-16T00:00:00 --until=2025-01-17T00:00:00)"
  assert_contains "amend: day 2 has commit" "$output_day2" "Initial work (amended)"
}

# ── Test 11: Rebase dedup with --until only (no --since) ─────────

test_dedup_until_only() {
  echo "-- test_dedup_until_only"
  setup_repo
  ...
  # Main gets a new commit, rebase feature onto it
  git checkout -q main
  echo "main work" > main.txt
  git add main.txt
  GIT_AUTHOR_DATE="2025-01-15 12:00:00 +0000" \
  ...
  # No --since, just --until: dedup should still work (no lookback needed)
  local output
  output="$(run_git_done --until=2025-01-17T00:00:00)"
  local count
  count="$(printf '%s' "$output" | grep -cF "Add feature work")"
  if [ "$count" -eq 1 ]; then
    PASS=$((PASS + 1))
    ...
    FAIL=$((FAIL + 1))
    echo "FAIL [until-only: should have commit exactly once]: found $count occurrences"
  fi
}

# ── Test 12: Lookback fetches original outside user range for dedup ─

test_lookback_dedup() {
  echo "-- test_lookback_dedup"
  setup_repo

  # Base commit
  echo "base" > file.txt
  git add file.txt
  GIT_AUTHOR_DATE="2025-01-01 10:00:00 +0000" \
  GIT_COMMITTER_DATE="2025-01-01 10:00:00 +0000" \
  git commit -q -m "Base commit"

  # Feature branch: commit on Jan 2
  git checkout -q -b feature
  echo "feature work" > feature.txt
  git add feature.txt
  GIT_AUTHOR_DATE="2025-01-02 10:00:00 +0000" \
  GIT_COMMITTER_DATE="2025-01-02 10:00:00 +0000" \
  git commit -q -m "Add feature work"
  local old_hash
  old_hash="$(git rev-parse HEAD)"

  # Main gets a commit on Jan 19
  git checkout -q main
  echo "main work" > main.txt
  git add main.txt
  GIT_AUTHOR_DATE="2025-01-19 10:00:00 +0000" \
  GIT_COMMITTER_DATE="2025-01-19 10:00:00 +0000" \
  git commit -q -m "Main work"

  # Rebase feature onto main on Jan 20 (committer date = Jan 20)
  git checkout -q feature
  GIT_COMMITTER_DATE="2025-01-20 09:00:00 +0000" \
  git rebase -q main
  git branch pre-rebase "$old_hash"

  # Query Jan 19–21: the rebased copy (committer Jan 20) is in range,
  # but the original (committer Jan 2) is outside --since.
  # Lookback (90 days) extends the fetch to include the original,
  # so dedup keeps the original (oldest) and discards the rebased copy.
  # After filtering to the user's range, the original is gone too → empty.
  local output
  output="$(run_git_done --since=2025-01-19T00:00:00 --until=2025-01-21T00:00:00)"
  assert_not_contains "lookback dedup: rebased copy removed" "$output" "Add feature work"

  # But the original should appear on its own day (Jan 2)
  local output_orig
  output_orig="$(run_git_done --since=2025-01-02T00:00:00 --until=2025-01-03T00:00:00)"
  assert_contains "lookback dedup: original on its day" "$output_orig" "Add feature work"
}
